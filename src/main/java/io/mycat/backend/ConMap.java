		...
		for (NIOProcessor processor: MycatServer.getInstance().getProcessors()) {
			for (BackendConnection con: processor.getBackends().values()) {
				if (con instanceof MySQLConnection) {
					...
					if (mysqlCon.getSchema().equals(schema) && mysqlCon.getPool() == dataSource && mysqlCon.isBorrowed()) {
						total++;
					}
                } else if (con instanceof JDBCConnection) {
                    JDBCConnection jdbcCon = (JDBCConnection) con;
                    if (jdbcCon.getSchema().equals(schema) && jdbcCon.getPool() == dataSource && jdbcCon.isBorrowed()) {
						total++;
                    }
                }
            }
		...
		for (NIOProcessor processor: MycatServer.getInstance().getProcessors()) {
			for (BackendConnection con: processor.getBackends().values()) {
				if (con instanceof MySQLConnection) {
                    ...
                    JDBCConnection jdbcCon = (JDBCConnection) con;
                    if (jdbcCon.getPool() == dataSource && jdbcCon.isBorrowed() && !jdbcCon.isClosed()) {
						total++;
                    }
                }
            }
