...
//    public int flushChannel(final AsynchronousSocketChannel channel, final ByteBuffer bb, final long writeTimeout) {
//        if (!bb.hasRemaining()) {
//            return 0;
//        }
//        int nWrite = bb.limit();
...
//    }
    ...
    private boolean write0() {
        ...
        if (theBuffer == null || !theBuffer.hasRemaining()) {// writeFinished,但要区分buffer是否NULL，不NULL，要回收
            if (theBuffer != null) {
    ...
    public void completed(final Integer result, final AIOSocketWR wr) {
        try {
            ...
            if (result >= 0) {
                ...
                wr.con.close("write error " + result);
            }
        } catch (Exception e) {
